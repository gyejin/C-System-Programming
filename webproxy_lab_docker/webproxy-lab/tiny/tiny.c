                 ...
                 char *shortmsg, char *longmsg)
{
    ...
    Rio_writen(fd, buf, strlen(buf));
    //HTTP헤더 만들 때, 보낼 HTML 본문(body)의 길이 헤더에 포함
    sprintf(buf, "Content-length: %d\r\n\r\n", (int)strlen(body)); 
    //헤더(buf)와 본문(body)을 실제로 네트워크를 통해 전송할 때도 전송 바이트 수를 알려줌
    Rio_writen(fd, buf, strlen(buf));
...
void get_filetype(char *filename, char *filetype)
{
    ...
    else if (strstr(filename, ".mpg"))      //숙제 11.7 MPG 비디오 파일 확장
...
int parse_uri(char *uri, char *filename, char *cgiargs)
{
    ...
    if (!strstr(uri, "cgi-bin")) {      //정적
        strcpy(cgiargs, "");
        strcpy(filename, ".");      //filename 변수를 .(현재 디렉토리)로 초기화
        strcat(filename, uri);      //   /home.html 같은 uri를 이어 붙여 ./home.html이라는 완전한 경로를 완성
        if (uri[strlen(uri)-1] == '/')
