...
static char *free_listp = NULL;     //가용 리스트 free_listp생성
...
void *mm_malloc(size_t size)
{
    ...
    //요청 사이즈가 8바이트보다 작으면
    if (size <= 2*DSIZE){     
        asize = 3*DSIZE;    //최소 블럭 크기인 24바이트로 세팅 (헤더 4 + 페이로드 16 + 푸터 4) => explicit에서는 최소블럭크기 24바이트 (페이로드에 PREV, SUCC 두개의 포인터)
    }
    //8바이트보다 크면
    else{
...
static void *coalesce(void *bp){
    ...
    if (prev_alloc && next_alloc){      //이전 블럭과 다음 블럭이 할당 돼있으면 합칠블록 없으니까
        mm_insert_block(bp);        //현재 블록 리스트에 추가
        return bp;      //현재 bp반환
    }

    else if (prev_alloc && !next_alloc){        //이전 블럭 = 할당, 다음블록 = 가용
        mm_remove_block(NEXT_BLKP(bp));     //다음 블록을 리스트에서 제거
        ...
        mm_insert_block(bp);        //합쳐진 블록 리스트에 추가
    }

    else if (!prev_alloc && next_alloc){        //이전 블록 = 가용, 다음블럭 = 할당 
        mm_remove_block(PREV_BLKP(bp));     //이전 블록을 리스트에서 제거
        ...
        mm_remove_block(NEXT_BLKP(bp)); //이전 블록 제거
        mm_remove_block(PREV_BLKP(bp)); //다음 블록 제거
        size += GET_SIZE(HDRP(PREV_BLKP(bp))) + GET_SIZE(HDRP(NEXT_BLKP(bp)));      //이전 다음 블럭 헤더 읽어와 사이즈 더해주고
        PUT(HDRP(PREV_BLKP(bp)), PACK(size, 0));        //이전 블럭의 헤더 업뎃
...
static void *find_first_fit(size_t asize){
    ...
    //가용리스트만 따라가면서 탐색, SUCC로 다음블록으로 하나씩 이동
    for (bp = free_listp; bp != NULL; bp = GET_SUCC(bp)){      
        if (asize <= GET_SIZE(HDRP(bp))){     //가용인건 당연, 크기 확인 담기에 충분하다면
            return bp;      //블록 포인터 반환
        }
    }
...
static void place(void *bp, size_t asize){
    ...
    mm_remove_block(bp);        //할당할거니까 가용리스트에서 제거
    ...
    if ((free_size - asize) >= (3*DSIZE)) {   
        ...
        mm_insert_block(bp);        //새로운 가용 블록 리스트에 추가
    }
